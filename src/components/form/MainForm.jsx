...
import { handleQuestionType } from '@/redux/store';

const type = [{ name: 'CheckBox' }, { name: 'TextArea' }];

export default function MainForm() {
  const dispatch = useDispatch();
  const selected = useSelector((state) => state.form.questionType);

  return (
    <div className="fixed top-16 w-72">
      <Listbox value={selected} onChange={(e) => dispatch(handleQuestionType(e))}>
